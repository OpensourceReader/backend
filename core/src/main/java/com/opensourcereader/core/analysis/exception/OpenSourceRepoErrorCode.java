...
public enum OpenSourceRepoErrorCode {
  ...
  REPOSITORY_CLONE_DIRECTORY_ALREADY_EXIST(
      HttpStatus.INTERNAL_SERVER_ERROR, "원격 저장소 클론 디렉토리가 이미 있습니다."),
  REPOSITORY_TREE_PARSE_FAILED(HttpStatus.INTERNAL_SERVER_ERROR, "로컬 Git 저장소의 트리 구조 파싱에 실패했습니다."),
  REPOSITORY_TREE_WALK_ACCESS_FAILED(
      HttpStatus.INTERNAL_SERVER_ERROR, "로컬 Git 저장소의 TreeWalk 객체를 불러올 없습니다."),
  ...
  REPOSITORY_AlREADY_EXIST(HttpStatus.INTERNAL_SERVER_ERROR, "저장소가 이미 있습니다."),

  FILE_SYSTEM_ROOT_DIRECTORY_NOT_DELETABLE(
      HttpStatus.INTERNAL_SERVER_ERROR, "루트 디렉토리는 삭제할 수 없습니다."),
  FILE_SYSTEM_DIRECTORY_CREATE_FAILED(HttpStatus.INTERNAL_SERVER_ERROR, "로컬 디렉터리 생성에 실패했습니다."),
  FILE_SYSTEM_DIRECTORY_DELETE_FAILED(HttpStatus.INTERNAL_SERVER_ERROR, "로컬 디렉터리 삭제에 실패했습니다.");
