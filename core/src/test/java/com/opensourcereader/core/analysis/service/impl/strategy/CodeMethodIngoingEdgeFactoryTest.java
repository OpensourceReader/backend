...
//
//    // when
//    List<CodeMethodCallEdge> edges = strategy.createIngoingEdges(repo.getId(), List.of("t/I"),
// caller);
//
//    // then
//    assertThat(edges)
...
//  @Test
//  @DisplayName(
//      "interface method가 DB에 없으면 external CodeMethod를 만들어 ingoing edge(interface -> caller)를
// 만든다(전략은 저장하지 않는다)")
//  void createIngoing_Edges_create_external_when_missing() {
//    // given
//    OpenSourceRepo repo = openSourceRepoRepository.save(new OpenSourceRepo("clone-url"));
//
//    CodeMethod caller = getCodeMethod("t/Main", "run", repo);
