...
class ExtensionTest {
  ...
  void resolveExtension_multipleDots() {
    SoftAssertions.assertSoftly(softly -> {
      softly.assertThat(Extension.resolveExtension("archive.tar.gz")).isEqualTo(Extension.OTHER);
      softly.assertThat(Extension.resolveExtension("my.test.file.java")).isEqualTo(Extension.JAVA);
    });
  }
