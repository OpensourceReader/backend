...
class ClassStructureExtractorTest {
   ...
   */
  private static List<Tuple> edgesOf(ClassStructure result) {
            ...
            ms ->
                ms.calleeMethods().stream()
                    .map(
                        callee ->
                            tuple(
                                ms.declaredMethodInfo().className(),
                                callee.className(),
                                callee.methodName(),
  ...
  private static void assertEdgesExactly(ClassStructure result, Tuple... expected) {
    assertThat(edgesOf(result)).containsExactlyInAnyOrder(expected);
  }

  private static ClassStructure analyzeSingle(String mainFqcn, String src) {
  ...
  private static ClassStructure analyzeSingle(String mainFqcn, Map<String, String> sources) {
  ...
  class ConstructorRules {
    ...
    void callGraphAnalyzerTest_no_init() {
      ...
      // when
      ClassStructure result = analyzeSingle("t.Main", src);

      // then
      assertSoftly(
          softly -> {
            softly.assertThat(result.classInfo().className()).isEqualTo("t/Main");
    ...
    void constructor_this_chaining_is_captured() {
      ...
      // when
      ClassStructure result = analyzeSingle("t.Main", sources);
  ...
  class PolymorphicDispatch {
    ...
    void super_call_is_captured() {
      ...
      // when
      ClassStructure result = analyzer.extract(List.of(new ClassBytecode(null, childBytes))).get(0);

      // then
      assertSoftly(
          softly -> {
            softly.assertThat(result.classInfo().className()).isEqualTo("t/Child");
